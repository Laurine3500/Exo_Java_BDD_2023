...
<% if (tableauDeChiffres.length > 0) { %>
    <% 
        int premiereValeur = Integer.parseInt(tableauDeChiffres[0]);
        int deuxValeur = Integer.parseInt(tableauDeChiffres[1]);
        int somme = premiereValeur + deuxValeur;
    %>
    <p>La somme des deux premières valeurs (<%= premiereValeur %>) et (<%= deuxValeur %>) est : <%= somme %></p>
<% } %>
